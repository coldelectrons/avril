...
static inline uint24c_t U24AddC( uint24c_t a, uint24_t b )
{
    ...
    asm( "add %0, %6"
         ...
         : "0"( a_frac ), "1"( a_int ), "2"( a_carry ), "a"( b_frac ),
           "a"( b_int ) );
...
static inline uint8_t U8Mix( uint8_t a, uint8_t b, uint8_t gain_a,
                             uint8_t gain_b )
{
    Word sum;
    asm( "mul %3, %4"
...
static inline int8_t S8U8MulShift8( int8_t a, uint8_t b ) { return a * b >> 8; }
...
static inline int8_t S8S8MulShift8( int8_t a, int8_t b ) { return a * b >> 8; }
                                         ...
                                         uint16_t phase )
{
    return U8Mix( pgm_read_byte( table + ( phase >> 8 ) ),
                  pgm_read_byte( 1 + table + ( phase >> 8 ) ), phase & 0xff );
}
