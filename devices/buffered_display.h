...
class BufferedDisplay {
   public:
    ...
    static void BlinkCursor() { ++blink_; }
    static void Tick()
    {
        // The following code is likely to write 4 bytes at most. If there are
        // less
        // than 4 bytes available for write in the output buffer, there's no
        // reason
        // to take the risk to continue.
        if ( Lcd::writable() < 4 ) {
            return;
        }
        // It is now safe to assume that all writes of 4 bytes to the display
        // buffer
        // will not block.
        ...
        // If the scan position is the cursor and it is shown (blinking), draw
        // the
        // cursor.
        if ( scan_position_ == cursor_position_ && ( blink_ & 128 ) ) {
            ...
            // displayed either on the left or right of the first line,
            // depending on
            // the available space.
            if ( status_ &&
                 ( scan_position_ == 0 || scan_position_ == ( width - 1 ) ) &&
        ...
        // Check whether the screen really has to be updated to show the
        // character.
        if ( character != remote_[scan_position_] ) {
            // There is a character to transmit!
            // If the new character to transmit is just after the previous one,
            // and on
            // the same line, we're good, we don't need to reposition the
            // cursor.
            if ( ( scan_position_ == scan_position_last_write_ + 1 ) &&
                ...
                // The character to transmit is at a different position, we need
                // to move
