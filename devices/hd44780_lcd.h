...
template <typename RsPin, typename EnablePin, typename ParallelPort,
          uint8_t width = 16, uint8_t height = 2>
class Hd44780Lcd {
   public:
    enum {
        buffer_size = 64,
    ...
    static inline void Init()
    {
        ...
        if ( height == 2 ) {
            SlowCommand( LCD_FUNCTION_SET | LCD_4_BITS | LCD_2_LINE |
                         LCD_SMALL_FONT );
        }
        else {
    ...
    static inline uint8_t Write( uint8_t character ) { WriteData( character ); }
    ...
    static inline void SetCustomCharMap( const uint8_t* data,
    ...
    static inline void SetCustomCharMapRes( const uint8_t* data,
                                            uint8_t num_characters,
                                            uint8_t first_character )
    {
        SlowCommand( LCD_SET_CGRAM_ADDRESS | ( first_character << 3 ) );
        for ( uint8_t i = 0; i < num_characters * 8; ++i ) {
            SlowData(
                SimpleResourcesManager::Lookup<uint8_t, uint8_t>( data, i ) );
        }
    }
...
volatile uint8_t
    Hd44780Lcd<RsPin, EnablePin, ParallelPort, width, height>::transmitting_;

/* static */
template <typename RsPin, typename EnablePin, typename ParallelPort,
          uint8_t width, uint8_t height>
volatile uint8_t
    Hd44780Lcd<RsPin, EnablePin, ParallelPort, width, height>::status_counter_;
